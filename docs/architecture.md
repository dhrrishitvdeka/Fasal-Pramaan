...
- **Key Subsystems**:
  ...
  - **On-Device Semantic Plant Classification & MobileNet v2 Gating**: The worker lazily loads **TF.js 4 plus `@tensorflow-models/mobilenet@2.1.1` (`load({ version: 2, alpha: 0.5 })`) from the jsdelivr CDN**; every ≤500 ms it classifies a dedicated 224×224 sample taken directly from the full-resolution video frame (the 64×64 buffer is reserved for the agronomic pixel math; the main thread ships both buffers to the worker, and the ImageBitmap path classifies straight from the source). Partitions ImageNet-1k classes into flora/crops vs. anthropogenic objects (suits, jerseys, t-shirts, rooms, walls, furniture, electronics); non-plant classes immediately trigger zeroed crop scores and shutter lock, while confirmed plant classes ($\ge 0.16$ probability) fuse with bio-optical micro-texture. The model verdict also gates `person_detected`: a person/human label ($\ge 0.2$) confirms the flag at a low skin-pixel ratio, a confident furniture/electronics or plant label vetoes it (desks, tables, and warm-brown laptops share the skin-tone color gamut), and the model-less fallback requires a strong ratio ($> 0.12$) plus a wood-grain exclusion. Hints include `ok`/`too_dark`/`too_bright`/`hold_steady`/`crop_not_detected`/`too_close`/`too_far`/`screen_detected`/`person_detected`.
...
- **Key Subsystems**:
  ...
  - **Sentinel-2 NDVI Trend Sparkline (`NdviTrendCard`)** (review detail): pure-SVG vegetation trend across the 90 days before → 30 days after the loss event (`claim.created_at`), amber dashed marker at the event. One Copernicus **Statistical API** request per claim (`/api/v1/statistics`, 5-day intervals, `maxCloudCoverage: 30`, intervals with dataMask mean < 0.5 dropped) behind the reviewer/owner-gated `GET /api/claims/{id}/satellite-trend` (30-min in-memory cache). Verdicts are conservative — `vegetation_collapse` needs ≥ 2 baseline points and a ≥ 20% mean NDVI drop; otherwise `no_significant_change` / `insufficient_data`. The card renders nothing when data is unavailable (no GPS, no token, or satellite fetch failure).
