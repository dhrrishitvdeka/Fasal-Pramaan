...
- **Key Subsystems**:
  ...
  - **On-Device Realtime CV** (`src/lib/vision/realtime-cv.ts` + `src/lib/vision/cv-worker.ts`): 64×64 canvas sampling runs off the main thread in a Web Worker. The worker lazily loads **TF.js 4 plus `@tensorflow-models/mobilenet@2.1.1` (`load({ version: 2, alpha: 0.5 })`) from the jsdelivr CDN**; every ≤500 ms it classifies a 224×224 upscale of the frame (top-3 predictions) and accepts the first class matching the plant regex (`plant|leaf|crop|grass|tree|flower|produce|vegetable|field|agricultur|maize|wheat|rice|paddy|corn…`) with **probability ≥ 0.18**. The final `cropDetected` is the union of the green-pixel heuristic (g > r+10 && g > b+10 && g > 60) and the model verdict; hints remain `ok`/`too_dark`/`too_bright`/`hold_steady`/`crop_not_detected`/`too_close`, and `shouldBlockShutter` never blocks when either signal says a crop is present. If TF.js/the weights are unavailable (offline, CSP), it degrades gracefully to heuristic-only mode — warn-once, never throws (hard stop after 3 consecutive inference failures).
