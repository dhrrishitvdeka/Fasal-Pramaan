...
| `REVIEWER_EMAILS` | **no** | yes for reviewers | Comma-separated emails treated as reviewers. All other Auth users are farmers. |
| `VOICE_ASSISTANT_ENABLED` | **no** | no | `true` to mint Fasal Saathi sessions on the hosted webapp |
| `GEMINI_API_KEY` | **no** | for Saathi | Server-only. Browser gets an ephemeral Live token only |
