...
$archive = Join-Path $output "FasalPramaan-portable.zip"
...
$excludedDirectories = @(
    ".git", ".agents", ".codex", ".ruff_cache", "dist", # excluded local metadata and caches
...
try {
    ...
    Set-Content -LiteralPath $checksum -Value "$archiveHash  FasalPramaan-portable.zip"
} finally {
