...
foreach ($container in @("fp-worker", "fp-beat")) {
    $running = docker inspect --format "{{.State.Running}}" $container 2>$null
    if ($LASTEXITCODE -ne 0 -or $running -ne "true") {
        throw "$container is not running. Run: docker logs --tail=100 $container"
    }
}

foreach ($container in @("fp-migrate", "fp-seed")) {
    $exitCode = docker inspect --format "{{.State.ExitCode}}" $container 2>$null
    if ($LASTEXITCODE -ne 0 -or $exitCode -ne "0") {
        throw "$container did not finish successfully. Run: docker logs $container"
    }
}
