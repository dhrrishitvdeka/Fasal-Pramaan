...
- Duplicate or exact same angle frames: If two or more uploaded photos are taken from the exact same standing spot/angle, show identical framing, or are duplicate frames → include "duplicate_angle" in quality_warnings, set predicted_grade="U", human_review_recommendation="recapture", and explicitly state which photos are duplicates in visual_findings and reasoning.
