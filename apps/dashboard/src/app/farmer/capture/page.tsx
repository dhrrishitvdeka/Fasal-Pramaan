...
function CaptureStudioContent() {
  ...
  const [selectedPlotId, setSelectedPlotId] = useState<string>(
    plotIdParam || (plots[0]?.id ?? "plot-101")
  );
  ...
  const [gpsCoords, setGpsCoords] = useState<{
    ...
    lon: number;
    accuracyM: number;
    status: "accurate" | "searching" | "simulated";
  }>({
    lat: selectedPlot?.lat || 27.8924,
    lon: selectedPlot?.lon || 76.2819,
    accuracyM: 2.8,
    status: "accurate",
  });
  ...
  useEffect(() => {
    if ("geolocation" in navigator) {
      const watchId = navigator.geolocation.watchPosition(
        ...
        (err) => {
          console.warn("GPS lookup error, using plot geofence centroid:", err);
          setGpsCoords({
            lat: selectedPlot?.lat || 27.8924,
            lon: selectedPlot?.lon || 76.2819,
            accuracyM: 3.2,
  ...
  const simulateVoiceInput = () => {
    ...
    setTimeout(() => {
      setIsListening(false);
      const sampleNotesHi = [
        "गुरुवार शाम की ओलावृष्टि से गेहूं की 70% बालियां टूट गईं और खेत के उत्तरी हिस्से में पीला रतुआ तेजी से फैल रहा है।",
        "सरसों की फलियों में सफेद रतुआ और माहू कीट का भारी प्रकोप देखा गया है, कतारों में पौधे गिर गए हैं।",
        "चने की फसल में फली छेदक सुंडी का हमला हुआ है जिससे लगभग 40% फलियां क्षतिग्रस्त हो गई हैं।",
      ];
      const sampleNotesEn = [
        "Heavy hailstorm on Thursday evening caused severe lodging and head shattering across the northern basin. Yellow rust pustules spreading rapidly.",
        "White rust lesions and heavy aphid infestation observed on mustard lower canopy. Stem breakage along eastern furrows.",
        "Pod borer caterpillar holes observed on developing chickpea pods with 40% crop defoliation.",
      ];
      const chosen =
        lang === "hi"
          ? sampleNotesHi[Math.floor(Math.random() * sampleNotesHi.length)]
          : sampleNotesEn[Math.floor(Math.random() * sampleNotesEn.length)];
      setObservations((prev) => (prev ? `${prev}\n${chosen}` : chosen));
      showToast(lang === "hi" ? "आवाज दर्ज की गई!" : "Voice recorded!");
    }, 1200);
  };

  // Helper to generate simulated SHA-256 hash
  ...
  const saveEvidenceImage = (imageUrl: string) => {
    const newEvidence: ClaimImageEvidence = {
      ...
      sha256: generateSimulatedHash(),
      qualityPassed: true,
      blurScore: 88 + Math.floor(Math.random() * 10),
      lightingScore: 90 + Math.floor(Math.random() * 8),
    };
  ...
  const handleSubmitClaim = async () => {
    ...
    try {
      ...
      if (isTargetedRecapture && recaptureClaimId) {
        // Targeted Recapture Submission
        const updated = updateClaimRecapture(recaptureClaimId, imagesList);
        setTimeout(() => {
          setIsSubmitting(false);
          router.push(`/farmer/claims/${recaptureClaimId}?recaptured=true`);
        }, 1000);
      } else {
        ...
        const newClaim = createClaim({
          ...
          cropVariety: selectedPlot.cropVariety,
          status: "under_review",
          farmerObservations: observations || "Field damage reported via 5-angle capture studio.",
          images: imagesList,
          evidenceTrust: {
            qualityScore: 94,
            coverageScore: 100,
            ...
            qualityNotes: "Crisp focus and balanced daylight across all 5 canonical views.",
            coverageNotes: "Full 5-angle set verified.",
            contextNotes: `GPS geofenced within ±${gpsCoords.accuracyM}m of Khasra ${selectedPlot.khasraNumber}.`,
            integrityNotes: "Device tamper check passed, SHA-256 certificate attached.",
          },
          aiPrediction: {
            cropIdentified: `${selectedPlot.cropType} (${selectedPlot.cropVariety})`,
            cropConfidence: 98.4,
            diseaseDetected:
              selectedPlot.cropType === "Wheat"
                ? "Yellow Rust (Puccinia striiformis)"
                : selectedPlot.cropType === "Mustard"
                ? "White Rust (Albugo candida)"
                : "Pod Borer Damage",
            diseaseDetectedHi:
              selectedPlot.cropType === "Wheat"
                ? "पीला रतुआ (पक्सीनिया स्ट्राइफोर्मिस)"
                : selectedPlot.cropType === "Mustard"
                ? "सफेद रतुआ (एल्बुगो कैंडिडा)"
                : "फली छेदक कीट क्षति",
            severityPercentage: 62,
            severityGrade: "High",
            affectedAreaHectares: Number((selectedPlot.areaHectares * 0.65).toFixed(2)),
            estimatedLossInr: Math.round(selectedPlot.areaHectares * 0.65 * 28000),
            modelConfidence: 94.6,
          },
          payoutStatus: "pending_review",
          payoutAmountInr: Math.round(selectedPlot.areaHectares * 0.65 * 28000),
        });
  ...
  return (
    <div className="mx-auto max-w-7xl px-3 py-4 sm:px-6 sm:py-6 space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        ...
        {/* Plot Selector (Disabled if in recapture mode) */}
        {!isTargetedRecapture && (
            ...
            >
              {plots.map((p) => (
                <option key={p.id} value={p.id}>
                  {lang === "hi" ? p.nameHi : p.name} ({p.khasraNumber})
                </option>
              ))}
            </select>
          </div>
