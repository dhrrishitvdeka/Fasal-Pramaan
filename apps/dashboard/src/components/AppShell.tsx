...
const farmerNav = [
  ...
  { href: "/farmer#new-claim", label: "File New Claim", hiLabel: "नया दावा दर्ज करें", icon: Camera },
  { href: "/farmer#tasks", label: "Pending Recaptures", hiLabel: "पुनः कैप्चर कार्य", icon: Layers },
  { href: "/farmer#advisor", label: "AI Crop Doctor", hiLabel: "AI फसल डॉक्टर", icon: Sparkles },
];
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const [roles, setRoles] = useState<string[]>(["reviewer", "administrator"]);
  ...
  useEffect(() => {
    ...
    async function verifySession() {
      ...
      try {
        ...
        // In demo showcase, fallback roles are kept
      }
      if (!cancelled) setReady(true);
    }
    void verifySession();
        ...
        <footer className="border-t border-slate-800 bg-slate-950 py-8 text-center text-xs text-slate-500">
          <div className="mx-auto max-w-7xl px-4 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="text-left">
              <span className="font-semibold text-slate-300">Fasal-Pramaan (फसल प्रमाण)</span> · MUN Interactive Showcase 2026
              <p className="text-[11px] text-slate-500 mt-0.5">Autonomous crop loss assessment, anti-fraud telemetry & PMFBY settlement engine</p>
            </div>
    ...
    <div className="flex min-h-screen bg-slate-50 text-slate-900">
      ...
      <aside className="flex w-60 shrink-0 flex-col border-r border-slate-200 bg-white">
        ...
        <div className="space-y-2 border-t border-slate-200 p-3">
          ...
          >
            <Home className="h-3.5 w-3.5" /> {lang === "hi" ? "शोकेस होम पेज" : "Showcase Home"}
          </Link>
          ...
          <div className="flex items-center justify-between gap-4 px-6 py-3">
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              <div className="text-right text-xs text-slate-400 border-l border-slate-200 pl-3 hidden md:block">
                <div className="font-semibold text-slate-700">PMFBY 2026</div>
                <div className="text-slate-500">Interactive Demo</div>
              </div>
            </div>
          </div>
        </header>
