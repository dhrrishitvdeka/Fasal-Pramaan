  ...
  Future<void> _restoreSession() async {
    ...
    try {
      ...
      if (me != null) {
        ...
        final roles = (me['roles'] as List?)?.map((r) => '$r').toList() ?? [];
        if (!mounted) return;
        if (roles.contains('field_officer')) {
          context.go('/officer');
        } else {
          context.go('/start?destination=${Uri.encodeQueryComponent('/home')}');
        }
        return;
      }
    } catch (_) {
    ...
    if (mounted) {
      context
          .go('/start?destination=${Uri.encodeQueryComponent('/onboarding')}');
    }
  }
