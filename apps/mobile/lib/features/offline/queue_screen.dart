...
class _QueueScreenState extends ConsumerState<QueueScreen> {
  ...
  Future<void> _sync() async {
    ...
    setState(() {
      isSyncing = true;
      status = isHi
          ? 'सर्वर के साथ ऑफ़लाइन साक्ष्य सिंक हो रहे हैं...'
          : 'Syncing offline evidence with server...';
    });
    try {
      final n = await sync.syncNow();
      await _load();
      setState(() => status = isHi
          ? '$n आइटम सफलतापूर्वक सिंक किए गए।'
          : 'Synced $n item(s). Local encrypted rows retained.');
    } catch (_) {
      setState(() => status = isHi
          ? 'सिंक में नेटवर्क समस्या आई। पुनः प्रयास हो रहा है...'
          : 'Sync encountered network issue. Retrying...');
    } finally {
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          status ??
                              '${items.length} ${isHi ? 'लंबित ऑफ़लाइन आइटम' : 'pending offline item(s)'}',
                          style: const TextStyle(
                            fontSize: 12,
