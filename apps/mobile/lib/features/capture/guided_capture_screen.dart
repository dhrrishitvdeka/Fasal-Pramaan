...
class _GuidedCaptureScreenState extends ConsumerState<GuidedCaptureScreen> {
  ...
  String getInstruction(bool isHi) {
    switch (angle) {
      ...
      case 'left_context':
        return isHi
            ? 'उसी जगह खड़े रहकर बाईं ओर का खेत और आसपास का संदर्भ लें।'
            : 'From the same position, turn left and include the crop boundary and nearby context.';
      case 'mid_canopy':
        return isHi
      ...
      case 'right_context':
        return isHi
            ? 'उसी जगह खड़े रहकर दाईं ओर का खेत और आसपास का संदर्भ लें।'
            : 'From the same position, turn right and include the crop boundary and nearby context.';
      case 'closeup_damage':
        return isHi
  ...
  void initState() {
    super.initState();
    voiceCaptureBridge.register(
      owner: this,
      capture: _captureFromVoice,
      guidance: _guidanceForVoice,
      save: _saveFromVoice,
      setObservation: _setObservationFromVoice,
    );
    _loadCycles();
  ...
  @override
  void dispose() {
    voiceCaptureBridge.unregister(this);
    cameraController?.dispose();
    observations.dispose();
    super.dispose();
  }

  Future<Map<String, dynamic>> _guidanceForVoice() async {
    final isHi = S.of(ref).isHi;
    return {
      'ok': true,
      'message': getInstruction(isHi),
      'angle': angle,
  ...
  Future<Map<String, dynamic>> _captureFromVoice() async {
    if (capturing) {
      return const {
        'ok': false,
        'message': 'A photo is already being captured. Please wait.',
      };
    }
    final requestedAngle = angle;
    await _captureCurrentAngle();
    final accepted = captures.containsKey(requestedAngle);
    return {
      'ok': accepted,
      'message': message ??
          (accepted
              ? 'Accepted the $requestedAngle photo.'
              : 'The $requestedAngle photo was not accepted.'),
  ...
  Future<Map<String, dynamic>> _setObservationFromVoice(
    String observation,
  ) async {
    observations.text = observation;
    observations.selection = TextSelection.collapsed(
      offset: observations.text.length,
    );
    if (mounted) setState(() {});
    return {
      'ok': true,
      'message': 'Saved the spoken observation on this capture draft.',
    };
  }

  Future<Map<String, dynamic>> _saveFromVoice() async {
    ...
    await _saveOffline(autoSubmit: false);
    return const {
      'ok': true,
      'message':
          'Saved the completed evidence as an encrypted offline draft. Upload still requires confirmation.',
    };
  }
  ...
  Future<void> _saveOffline({bool autoSubmit = true}) async {
    final s = S.of(ref);
    final isHi = s.isHi;
    if (selectedCycleId == null || selectedCycleId!.isEmpty) {
      setState(() => message = isHi
          ? 'कृपया पहले फसल चक्र चुनें।'
    ...
    if (autoSubmit) {
      if (mounted) {
        setState(() {
          submitting = true;
          message = isHi
              ? 'साक्ष्य अपलोड हो रहा है और स्थानीय वर्गीकरण शुरू हो रहा है…'
              : 'Uploading evidence and starting local classification…';
        });
      }
      final synced = await SyncService(api, db).syncNow();
      if (!mounted) return;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            synced > 0
                ? (isHi
                    ? 'साक्ष्य अपलोड हो गया। वर्गीकरण के बाद यह समीक्षक डैशबोर्ड पर दिखाई देगा।'
                    : 'Evidence uploaded. After local classification it will appear on the reviewer dashboard.')
                : (isHi
                    ? 'साक्ष्य सुरक्षित कतार में है। कनेक्शन मिलते ही यह अपने आप अपलोड होगा।'
                    : 'Evidence is safely queued and will upload automatically when connectivity returns.'),
          ),
      ...
      SnackBar(
        content: Text(
          isHi
              ...
              : 'Saved encrypted evidence offline. Confirm upload by voice when ready.',
        ),
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.recaptureSubmissionId == null
            ? (isHi
                ? 'मार्गदर्शित फसल साक्ष्य कैप्चर'
                : 'Guided Crop Evidence Capture')
      ...
      body: ListView(
        ...
        children: [
          ...
          TextField(
            ...
            decoration: InputDecoration(
              labelText: isHi
                  ? 'किसान की टिप्पणी (वैकल्पिक)'
                  : 'Farmer Observations (Optional)',
