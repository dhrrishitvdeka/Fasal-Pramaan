...
@celery_app.task(name="generate_evidence_reminders")
def generate_evidence_reminders() -> dict[str, int]:
    """Create due/overdue in-app reminders without duplicating deliveries."""
    from app.services.evidence_reminders import dispatch_due_evidence_reminders

    with SessionLocal() as db:
        return {"notifications_created": dispatch_due_evidence_reminders(db)}
