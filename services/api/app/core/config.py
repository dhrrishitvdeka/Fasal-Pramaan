...
class Settings(BaseSettings):
    ...
    celery_queue_reminders: str = "reminders"
    ...
    def assert_safe_for_environment(self) -> None:
        ...
        if (
            self.voice_assistant_enabled
            and not self.gemini_api_key.get_secret_value().strip()
        ):
            raise RuntimeError(
                "GEMINI_API_KEY is required when VOICE_ASSISTANT_ENABLED=true"
            )
