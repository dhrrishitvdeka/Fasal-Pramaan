...
class EvidenceReminderPlan(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    """Recurring geo-tagged evidence schedule for one farmer crop cycle."""

    __tablename__ = "evidence_reminder_plans"
    __table_args__ = (
        UniqueConstraint("user_id", "crop_cycle_id", name="uq_evidence_reminder_user_cycle"),
        Index("ix_evidence_reminders_due", "is_active", "next_due_at"),
    )
    ...
    cadence_days: Mapped[int] = mapped_column(Integer, default=30, nullable=False)
    target_photos: Mapped[int] = mapped_column(Integer, default=5, nullable=False)
