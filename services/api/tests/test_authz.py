...
def test_field_officer_is_scoped_to_assigned_jurisdiction(client: TestClient):
    from app.db.models import Farm, FarmerProfile, FieldOfficerProfile, Jurisdiction, User
    from app.db.session import SessionLocal

    code = f"OUT-{uuid.uuid4().hex[:10]}"
    state_code = f"STATE-{uuid.uuid4().hex[:10]}"
    assigned_code = f"ASSIGNED-{uuid.uuid4().hex[:10]}"
    with SessionLocal() as db:
        state = Jurisdiction(code=state_code, name="Test State", level="state")
        db.add(state)
        db.flush()
        assigned = Jurisdiction(
            code=assigned_code,
            name="Assigned Officer Scope",
            level="block",
            parent_id=state.id,
        )
        db.add(assigned)
        ...
        officer_profile = (
            ...
            .filter(User.email == "officer@fasalpramaan.local")
            .one()
        )
        officer_profile.jurisdiction_id = assigned.id
        farmer = (
    ...
    try:
        ...
        with SessionLocal() as db:
            ...
            db.query(FieldOfficerProfile).filter(
                FieldOfficerProfile.jurisdiction_id == db.query(Jurisdiction.id).filter(
                    Jurisdiction.code == assigned_code
                ).scalar_subquery()
            ).update({FieldOfficerProfile.jurisdiction_id: None}, synchronize_session=False)
            db.query(Jurisdiction).filter(Jurisdiction.code == code).delete(synchronize_session=False)
            db.query(Jurisdiction).filter(Jurisdiction.code == assigned_code).delete(synchronize_session=False)
            db.query(Jurisdiction).filter(Jurisdiction.code == state_code).delete(synchronize_session=False)
            db.commit()
